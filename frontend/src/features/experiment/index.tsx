...
export function ExperimentDashboard() {
  ...
  useEffect(() => {
    // Initialize default active trades if empty
  ...
  const handleRunBacktest = async (config: {
    ...
    try {
      ...
      const requestPayload = {
        ...
        instances: config.instances.length > 0 ? config.instances : [{ type: 'MA' }],
        policy: config.policy,
      };
      console.log('Sending startSearch request:', requestPayload);
      const started = await startSearch(requestPayload);
      ...
      setIsLoading(false);
      console.error('startSearch failed:', error);
