...
The unauthenticated screen offers an explicit offline demo. In authenticated
`LIVE` mode, chart/API failures are shown as errors and never silently replaced
with generated candles or trades. Simulated data exists only in explicit
`DEMO` mode.
...
Vite proxies `/auth`, `/markets`, `/candles`, `/strategies`, `/search`, `/experiments`,
...
The app authenticates before opening WebSocket. The API market catalog drives
all chart, trade-feed, strategy-search and backtest selectors. Market history,
realtime candles, Binance aggregate trades, strategy registry, search and
leaderboard use backend data. Active candle/trade subscriptions are restored
